...
describe('FeatureServiceRegistry', () => {
    let featureServiceRegistry;
    let mockExternalsValidator;
    let providerDefinitionA;
    let providerDefinitionB;
    let providerDefinitionC;
    let binderA;
    let binderB;
    let binderC;
    let bindingA;
    ...
    beforeEach(() => {
        ...
        featureServiceRegistry = new __1.FeatureServiceRegistry({ logger: logger_1.logger });
    ...
    describe('#registerFeatureServices', () => {
        function testRegistrationOrderABC() {
        ...
        it('fails to register the Feature Service "c" due to the lack of dependency "a"', () => {
            expect(() => featureServiceRegistry.registerFeatureServices([providerDefinitionC], 'test')).toThrowError(new Error('The required Feature Service "a" is not registered and therefore could not be bound to consumer "c".'));
        });
        it('does not fail to register the Feature Service "b" due to the lack of optional dependency "a"', () => {
            ...
            expect(() => featureServiceRegistry.registerFeatureServices([providerDefinitionB], 'test')).not.toThrowError();
            expect(logger_1.logger.info.mock.calls).toEqual([
                [
        ...
        it('fails to register a Feature Service due to an unsupported dependency version', () => {
            ...
            expect(() => featureServiceRegistry.registerFeatureServices([providerDefinitionA, stateProviderD], 'test')).toThrowError(new Error('The required Feature Service "a" in the unsupported version range "^2.0.0" could not be bound to consumer "d". The supported versions are ["1.1.0"].'));
        });
        it('does not fail to register a Feature Service due to an unsupported optional dependency version', () => {
            ...
            expect(() => featureServiceRegistry.registerFeatureServices([providerDefinitionA, stateProviderD], 'test')).not.toThrowError();
            expect(logger_1.logger.info.mock.calls).toEqual([
                [
                    'The Feature Service "a" has been successfully registered by registrant "test".',
                ],
        ...
        it('fails to register a Feature Service due to an invalid dependency version', () => {
            ...
            expect(() => featureServiceRegistry.registerFeatureServices([providerDefinitionA, stateProviderDefinitionD], 'test')).toThrowError(new Error('The required Feature Service "a" in an invalid version could not be bound to consumer "d".'));
        });
        it('does not fail to register a Feature Service due to an invalid optional dependency version', () => {
            ...
            expect(() => featureServiceRegistry.registerFeatureServices([providerDefinitionA, stateProviderDefinitionD], 'test')).not.toThrowError();
            expect(logger_1.logger.info.mock.calls).toEqual([
                [
                    'The Feature Service "a" has been successfully registered by registrant "test".',
                ],
        ...
        it('fails to register a Feature Service that provides an invalid version', () => {
            ...
            expect(() => featureServiceRegistry.registerFeatureServices([stateProviderDefinitionD], 'test')).toThrowError(new Error('The Feature Service "d" could not be registered by registrant "test" because it defines the invalid version "2.0".'));
        });
        ...
        describe('with an ExternalsValidator provided to the FeatureServiceRegistry', () => {
            beforeEach(() => {
                featureServiceRegistry = new __1.FeatureServiceRegistry({
    ...
    describe('#bindFeatureServices', () => {
        ...
        it('fails to create a bindings object for a consumer which is already bound', () => {
            ...
            expect(() => featureServiceRegistry.bindFeatureServices({}, 'foo')).toThrowError(new Error('All required Feature Services are already bound to consumer "foo".'));
        });
        describe('#unbind', () => {
            it('unbinds the consumer', () => {
                ...
                bindings.unbind();
                expect(() => featureServiceRegistry.bindFeatureServices(providerDefinitionA, providerDefinitionA.id)).not.toThrowError();
            });
            ...
            it('fails to unbind an already unbound consumer', () => {
                const bindings = featureServiceRegistry.bindFeatureServices(providerDefinitionA, providerDefinitionA.id);
                bindings.unbind();
                expect(() => bindings.unbind()).toThrowError(new Error('All required Feature Services are already unbound from consumer "a".'));
            });
