...
describe('ExternalsValidator', () => {
    describe('#constructor', () => {
        it('throws an error for provided externals containing an invalid version', () => {
            expect(() => new externals_validator_1.ExternalsValidator({ react: 'invalid' })).toThrowError(new Error('The provided version "invalid" for the external "react" is invalid.'));
        });
        it('throws an error for provided externals containing a coercable version', () => {
            expect(() => new externals_validator_1.ExternalsValidator({ react: '2.0' })).toThrowError(new Error('The provided version "2.0" for the external "react" is invalid.'));
        });
    });
    describe('#validate', () => {
        it("doesn't throw for a satisfied required external", () => {
            const validator = new externals_validator_1.ExternalsValidator({ react: '16.6.0' });
            expect(() => {
        ...
        it('throws an error for an unsatisfied required external (not provided)', () => {
            const validator = new externals_validator_1.ExternalsValidator({});
            expect(() => {
                validator.validate({ react: '^16.7.0' });
...
//# sourceMappingURL=externals-validator.test.js.map