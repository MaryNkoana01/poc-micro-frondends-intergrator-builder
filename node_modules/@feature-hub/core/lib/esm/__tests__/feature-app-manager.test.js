...
import { FeatureAppManager, } from '..';
import { logger } from './logger';
describe('FeatureAppManager', () => {
    let featureAppManager;
    let mockFeatureServiceRegistry;
    let mockFeatureServicesBinding;
    let mockExternalsValidator;
    let mockFeatureServicesBindingUnbind;
    let mockModuleLoader;
    let mockFeatureAppDefinition;
    let mockFeatureAppModule;
    ...
    describe('#getAsyncFeatureAppDefinition', () => {
        ...
        describe('with a custom module loader that handles multiple module types', () => {
            let mockFeatureAppDefinitionA;
            let mockFeatureAppDefinitionB;
            let mockFeatureAppModuleA;
            let mockFeatureAppModuleB;
            beforeEach(() => {
                ...
                featureAppManager = new FeatureAppManager(mockFeatureServiceRegistry, {
                    logger,
                    moduleLoader: async (_url, moduleType) => moduleType === 'a'
            ...
            it('returns an async value containing the Feature App definition for the correct module type', async () => {
                ...
                const asyncFeatureAppDefinitionA = featureAppManager.getAsyncFeatureAppDefinition(url, 'a');
                ...
                const asyncFeatureAppDefinitionB = featureAppManager.getAsyncFeatureAppDefinition(url, 'b');
            ...
            it('returns an async value containing an error for an missing module type', async () => {
                const expectedError = new Error('The Feature App module at the url "/example.js" with no specific module type is invalid. A Feature App module must have a Feature App definition as default export. A Feature App definition is an object with at least a `create` method. Hint: The provided module loader expects an optional second parameter `moduleType`. It might need to be provided for this Feature App.');
                await expect(featureAppManager.getAsyncFeatureAppDefinition('/example.js').promise).rejects.toEqual(expectedError);
            });
            it('returns an async value containing an error for an unknown module type', async () => {
                const expectedError = new Error('The Feature App module at the url "/example.js" with the module type "unknown" is invalid. A Feature App module must have a Feature App definition as default export. A Feature App definition is an object with at least a `create` method.');
                await expect(featureAppManager.getAsyncFeatureAppDefinition('/example.js', 'unknown').promise).rejects.toEqual(expectedError);
            });
        });
        describe.each([
            undefined,
        ...
        ])('when an invalid Feature App module (%o) has been loaded', (invalidFeatureAppModule) => {
            beforeEach(() => {
                // tslint:disable-next-line:no-any
                mockFeatureAppModule = invalidFeatureAppModule;
            });
            it('throws an error (and stores it on the async value)', async () => {
                const expectedError = new Error('The Feature App module at the url "/example.js" with no specific module type is invalid. A Feature App module must have a Feature App definition as default export. A Feature App definition is an object with at least a `create` method.');
                await expect(featureAppManager.getAsyncFeatureAppDefinition('/example.js')
                    .promise).rejects.toEqual(expectedError);
                expect(featureAppManager.getAsyncFeatureAppDefinition('/example.js').error).toEqual(expectedError);
            });
        });
        it('throws an error if no module loader was provided', () => {
            ...
            expect(() => featureAppManager.getAsyncFeatureAppDefinition('/example.js')).toThrowError(new Error('No module loader provided.'));
        });
        describe('for a known Feature App module url', () => {
            it('returns the same async Feature App definition', () => {
                const asyncFeatureAppDefinition = featureAppManager.getAsyncFeatureAppDefinition('/example.js');
                expect(featureAppManager.getAsyncFeatureAppDefinition('/example.js')).toBe(asyncFeatureAppDefinition);
            });
        });
    });
    describe('#createFeatureAppScope', () => {
        it('creates a Feature App with a consumer environment using the Feature Service registry', () => {
            ...
            featureAppManager.createFeatureAppScope(featureAppId, mockFeatureAppDefinition, { baseUrl, config });
        ...
        describe('with a beforeCreate callback', () => {
            ...
            describe('that throws an error', () => {
                it('throws the same error and unbinds the bound Feature Services', () => {
                    ...
                    expect(() => featureAppManager.createFeatureAppScope('testId', mockFeatureAppDefinition, {
                        beforeCreate: () => {
                            throw mockError;
                        },
        ...
        describe('with a done callback', () => {
            it('passes the done callback as part of the env to the Feature App', () => {
                ...
                featureAppManager.createFeatureAppScope(featureAppId, mockFeatureAppDefinition, { done: mockDone });
                const { featureServices } = mockFeatureServicesBinding;
                expect(mockFeatureAppCreate.mock.calls).toEqual([
        ...
        describe('without an ExternalsValidator provided to the FeatureAppManager', () => {
            describe('with a Feature App definition that is declaring external dependencies', () => {
                beforeEach(() => {
                    mockFeatureAppDefinition = Object.assign(Object.assign({}, mockFeatureAppDefinition), { dependencies: {
                            externals: {
                                react: '^16.0.0',
                            },
                        } });
        ...
        describe('when bindFeatureServices throws an error', () => {
            ...
            it('throws the same error', () => {
                expect(() => featureAppManager.createFeatureAppScope('testId', mockFeatureAppDefinition)).toThrowError(mockError);
            });
        });
        ...
        describe('for a known Feature App definition', () => {
            describe('with a beforeCreate callback', () => {
                it('does not call the beforeCreate callback multiple times', () => {
                    const featureAppId = 'testId';
                    featureAppManager = new FeatureAppManager(mockFeatureServiceRegistry, { logger });
                    const mockBeforeCreate = jest.fn();
                    featureAppManager.createFeatureAppScope(featureAppId, mockFeatureAppDefinition, { beforeCreate: mockBeforeCreate });
                    featureAppManager.createFeatureAppScope(featureAppId, mockFeatureAppDefinition, { beforeCreate: mockBeforeCreate });
