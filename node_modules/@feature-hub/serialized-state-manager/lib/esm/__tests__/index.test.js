...
import { defineSerializedStateManager, serializedStateManagerDefinition, } from '..';
describe('defineSerializedStateManager', () => {
    ...
    it('defines an id', () => {
        expect(serializedStateManagerDefinition.id).toBe('s2:serialized-state-manager');
    });
    it('has no dependencies', () => {
        expect(serializedStateManagerDefinition.dependencies).toBeUndefined();
        expect(serializedStateManagerDefinition.optionalDependencies).toBeUndefined();
    });
    describe('#create', () => {
        it('creates a shared Feature Service containing version 1.0.0', () => {
            const sharedSerializedStateManager = serializedStateManagerDefinition.create(mockEnv);
    ...
    describe('SerializedStateManagerV1', () => {
        ...
        beforeEach(() => {
            const serializedStateManagerBinder = serializedStateManagerDefinition.create(mockEnv)['1.0.0'];
        ...
        describe('#getSerializedState', () => {
            ...
            describe('when the integrator has passed serialized states into the definition factory function', () => {
                ...
                beforeEach(() => {
                    const serializedStateManagerBinder = defineSerializedStateManager(JSON.stringify({ 'test:consumer:1': serializedStateConsumer1 })).create(mockEnv)['1.0.0'];
                    consumer1SerializedStateManager =
                        serializedStateManagerBinder('test:consumer:1').featureService;
                    consumer2SerializedStateManager =
                        serializedStateManagerBinder('test:consumer:2').featureService;
