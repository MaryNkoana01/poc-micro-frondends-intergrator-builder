...
const __1 = require("..");
const root_location_helpers_1 = require("./root-location-helpers");
const stubbed_logger_1 = require("./stubbed-logger");
const simulateOnPopState = (state, url) => {
...
describe('defineHistoryService', () => {
    ...
    beforeEach(() => {
        ...
        historyServiceDefinition = (0, __1.defineHistoryService)({
    ...
    describe('HistoryServiceV1', () => {
        ...
        beforeEach(() => {
            mockEnv = { featureServices: { 's2:logger': stubbed_logger_1.stubbedLogger } };
            createHistoryServiceBinder = () => {
                const sharedHistoryService = (0, __1.defineHistoryService)((0, __1.createRootLocationTransformer)({ consumerPathsQueryParamName: root_location_helpers_1.consumerPathsQueryParamName })).create(mockEnv);
        ...
        describe('#createBrowserHistory()', () => {
            ...
            describe('when called multiple times for the same consumer', () => {
                it('returns the same instance and logs a warning', () => {
                    expect(historyBinding1.featureService.createBrowserHistory()).toEqual(history1);
                    expect(stubbed_logger_1.stubbedLogger.warn).toHaveBeenCalledWith('createBrowserHistory was called multiple times by consumer "test1". Returning the same history instance as before.');
                });
            });
            describe('#length', () => {
            ...
            describe('#location', () => {
                ...
                it('retrieves consumer specific locations from the initial location', () => {
                    ...
                    const url = (0, root_location_helpers_1.createUrl)({ test1: '/foo#some-anchor', test2: 'bar' });
            ...
            describe('#push()', () => {
                it('pushes a new root location onto the root history for every consumer push', () => {
                    ...
                    expect(window.location.href).toBe((0, root_location_helpers_1.createUrl)({ test1: '/foo', test2: '/bar?baz=1' }));
                ...
                it('normalizes the pathname', () => {
                    history1.push('foo');
                    expect(window.location.href).toBe((0, root_location_helpers_1.createUrl)({ test1: '/foo' }));
                });
                it('handles relative pathnames', () => {
                    history1.push('/foo/bar');
                    history1.push('baz');
                    expect(window.location.href).toBe((0, root_location_helpers_1.createUrl)({ test1: '/foo/baz' }));
                });
            });
            describe('#replace()', () => {
            ...
            describe('#go()', () => {
                it('does nothing and logs a warning that go() is not supported', () => {
                    ...
                    expect(stubbed_logger_1.stubbedLogger.warn).toHaveBeenCalledWith('history.go() is not supported.');
                });
            });
            describe('#goBack()', () => {
                it('does nothing and logs a warning that goBack() is not supported', () => {
                    ...
                    history1.goBack();
                    expect(window.location.href).toBe(href);
                    expect(stubbed_logger_1.stubbedLogger.warn).toHaveBeenCalledWith('history.goBack() is not supported.');
                });
            });
            describe('#goForward()', () => {
                it('does nothing and logs a warning that goForward() is not supported', () => {
                    ...
                    history1.goForward();
                    expect(window.location.href).toBe(href);
                    expect(stubbed_logger_1.stubbedLogger.warn).toHaveBeenCalledWith('history.goForward() is not supported.');
                });
            });
            describe('#block()', () => {
                it('does nothing and logs a warning', () => {
                    ...
                    expect(window.location.href).toBe((0, root_location_helpers_1.createUrl)({ test1: '/foo?bar=1' }));
                    expect(promptHookSpy).not.toHaveBeenCalled();
                    expect(stubbed_logger_1.stubbedLogger.warn).toHaveBeenCalledWith('history.block() is not supported.');
                });
            ...
            describe('#listen()', () => {
                it('calls listeners only for consumer-specific history changes', () => {
                    ...
                    expect(listenerSpy1).toHaveBeenCalledWith(expect.objectContaining({ pathname: '/foo', search: '?bar=1' }), 'PUSH');
                    expect(listenerSpy2).toHaveBeenCalledTimes(1);
                    expect(listenerSpy2).toHaveBeenCalledWith(expect.objectContaining({ pathname: '/baz', search: '?qux=2' }), 'REPLACE');
                });
                ...
                describe('for a POP action', () => {
                    let history1ListenerSpy;
                    let history2ListenerSpy;
                    ...
                    it('calls listeners only for matching consumer locations', () => {
                        ...
                        simulateOnPopState(state, href);
                        expect(history1ListenerSpy).toHaveBeenCalledWith(expect.objectContaining({ pathname: '/' }), 'POP');
                    ...
                    describe('with back and forward navigation', () => {
                        it('calls the listener with the correct locations', () => {
                            ...
                            expect(history1ListenerSpy).toHaveBeenCalledWith(expect.objectContaining({ pathname: '/baz', search: '?qux=3' }), 'POP');
                            history1ListenerSpy.mockClear();
                            simulateOnPopState(state2, href2); // POP foward
                            expect(history1ListenerSpy).toHaveBeenCalledWith(expect.objectContaining({ pathname: '/bar', search: '?qux=4' }), 'POP');
                        });
                    });
                    describe('when a location is replaced', () => {
                        it('calls the listener with the correct locations', () => {
                            ...
                            expect(history2ListenerSpy).toHaveBeenCalledWith(expect.objectContaining({ pathname: '/b2' }), 'POP');
                        });
                    });
                    describe('after re-initializing the history service, e.g. because of a page reload', () => {
                        it('calls listeners for matching consumer locations (also restores consumer state)', () => {
                            ...
                            registerListeners();
                            simulateOnPopState(state, href);
                            expect(history1ListenerSpy).toHaveBeenCalledWith(expect.objectContaining({
                                pathname: '/foo',
                                search: '?a=1',
                                state: consumerState,
                            }), 'POP');
            ...
            describe('#createHref()', () => {
                it('returns the href for the given location based on the root browser history', () => {
                    ...
                    expect(href).toBe((0, root_location_helpers_1.createUrl)({ test1: '/foo', test2: '/bar?a=b' }, { relative: true }));
                });
                it('normalizes the pathname', () => {
                    history1.push('/foo');
                    const location = { pathname: 'bar' };
                    const href = history2.createHref(location);
                    expect(href).toBe((0, root_location_helpers_1.createUrl)({ test1: '/foo', test2: '/bar' }, { relative: true }));
                });
            });
        ...
        describe('when no Logger Feature Service is provided', () => {
            ...
            it('logs messages using the console', () => {
                ...
                browserHistory.go(-1);
                expect(consoleWarnSpy).toHaveBeenCalledWith('history.go() is not supported.');
            });
        });
    });
    describe('HistoryServiceV2', () => {
        ...
        beforeEach(() => {
            mockEnv = {
                featureServices: { 's2:logger': stubbed_logger_1.stubbedLogger },
            };
            createHistoryServiceBinder = () => {
                const sharedHistoryService = (0, __1.defineHistoryService)((0, __1.createRootLocationTransformer)({ consumerPathsQueryParamName: root_location_helpers_1.consumerPathsQueryParamName })).create(mockEnv);
        ...
        describe('#history', () => {
            ...
            describe('#location', () => {
                ...
                it('retrieves consumer specific locations from the initial location', () => {
                    ...
                    const url = (0, root_location_helpers_1.createUrl)({ test1: '/foo', test2: 'bar' });
                    window.history.pushState({ usr: consumerStates }, '', url);
                    createHistories();
                    expect(history1.location).toMatchObject({
                        pathname: '/foo',
            ...
            describe('#location', () => {
                it('returns the root location containing all consumer locations', () => {
                    ...
                    expect(historyService1.rootHistory.location).toMatchObject({
                        pathname: '/',
                        search: (0, root_location_helpers_1.createSearch)({ test1: '/foo', test2: '/bar' }),
                        state: {
                ...
                describe('for a POP action', () => {
                    it('returns the previous root location', () => {
                        ...
                        expect(historyService1.rootHistory.location).toMatchObject({
                            pathname: '/',
                            search: (0, root_location_helpers_1.createSearch)({ test1: '/foo' }),
                            state: state.usr,
                        });
                    });
                });
            ...
            describe('#listen()', () => {
                it('calls listeners for consumer history changes', () => {
                    ...
                    expect(listenerSpy.mock.calls).toMatchObject([
                        [
                            {
                                pathname: '/',
                                search: (0, root_location_helpers_1.createSearch)({ test1: '/bar' }),
                ...
                describe('for a POP action', () => {
                    it('calls listeners with a POP event', () => {
                        ...
                        historyService1.rootHistory.listen(listenerSpy);
                        simulateOnPopState(state, href);
                        expect(listenerSpy).toHaveBeenCalledWith(expect.objectContaining({ pathname: '/' }), 'POP');
                    });
                });
            });
            describe('#createHref()', () => {
        ...
        describe('#createNewRootLocationForMultipleConsumers', () => {
            it('creates a new root location for a single consumer', () => {
                ...
                expect(location).toMatchObject({
                    pathname: '/',
                    search: (0, root_location_helpers_1.createSearch)({ test1: '/test1' }),
            ...
            it('creates a new root location for two consumers', () => {
                ...
                expect(location).toMatchObject({
                    pathname: '/',
                    search: (0, root_location_helpers_1.createSearch)({ test1: '/test', test2: '/xxx' }),
                    state: {
            ...
            describe('with a createNewRootLocationForMultipleConsumers method defined in the root location transformer', () => {
                let createNewRootLocationForMultipleConsumersMock;
                let historyServiceTest1;
                beforeEach(() => {
                    createNewRootLocationForMultipleConsumersMock = jest.fn(() => ({
                        pathname: 'root-test',
                    }));
                    createHistoryServiceBinder = () => {
                        const sharedHistoryService = (0, __1.defineHistoryService)({
                            createRootLocation: jest.fn(),
                            getConsumerPathFromRootLocation: jest.fn(),
                ...
                it('uses the custom createNewRootLocationForMultipleConsumers method', () => {
                    ...
                    historyServiceTest1.createNewRootLocationForMultipleConsumers(consumerLocation);
                    expect(createNewRootLocationForMultipleConsumersMock.mock.calls).toEqual([[consumerLocation]]);
                });
                it('sets the consumer states on the new root location', () => {
                    ...
                    const location = historyServiceTest1.createNewRootLocationForMultipleConsumers(consumerLocation);
                    expect(location).toMatchObject({
                        pathname: 'root-test',
                        search: '',
                        hash: '',
    ...
    describe('HistoryServiceV3', () => {
        ...
        beforeEach(() => {
            ...
            createHistoryServiceBinder = (getHistoryKey) => {
                const sharedHistoryService = (0, __1.defineHistoryService)((0, __1.createRootLocationTransformer)({ consumerPathsQueryParamName: root_location_helpers_1.consumerPathsQueryParamName }), { getHistoryKey }).create(mockEnv);
        ...
        describe('#historyKey', () => {
            ...
            describe('with a custom `getHistoryKey` function', () => {
                it('uses the result of `getHistoryKey`', () => {
                    ...
                    const historyServiceBinder = createHistoryServiceBinder(getHistoryKeyMock);
        ...
        describe('#history', () => {
            ...
            describe('#back()', () => {
                it('does nothing and logs a warning that back() is not supported', () => {
                    ...
                    history1.back();
                    expect(window.location.href).toBe(href);
                    expect(stubbed_logger_1.stubbedLogger.warn).toHaveBeenCalledWith('history.back() is not supported.');
                });
            });
            describe('#forward()', () => {
                it('does nothing and logs a warning that forward() is not supported', () => {
                    ...
                    history1.forward();
                    expect(window.location.href).toBe(href);
                    expect(stubbed_logger_1.stubbedLogger.warn).toHaveBeenCalledWith('history.forward() is not supported.');
                });
            });
            describe('#block()', () => {
                it('does nothing and logs a warning', () => {
        ...
        describe('#rootHistory', () => {
            ...
            describe('#push()', () => {
                ...
                it('changes the location and action of affected consumer histories', () => {
                    const _a = historyService1.createNewRootLocationForMultipleConsumers({
                        historyKey: 'test2',
                        location: { pathname: '/test2' },
                    }), { state } = _a, to = __rest(_a, ["state"]);
