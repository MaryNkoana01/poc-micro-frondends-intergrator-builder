    ...
    describe('HistoryServiceV1 (on Node.js)', () => {
        ...
        beforeEach(() => {
            ...
            createHistoryServiceBinder = () => {
                const sharedHistoryService = defineHistoryService(createRootLocationTransformer({ consumerPathsQueryParamName }), { mode: 'static' }).create(mockEnv);
        ...
        describe('#createStaticHistory()', () => {
            ...
            describe('when called multiple times for the same consumer', () => {
                it('returns the same instance and logs a warning', () => {
                    ...
                    expect(stubbedLogger.warn).toHaveBeenCalledWith('createStaticHistory was called multiple times by consumer "test1". Returning the same history instance as before.');
                });
            });
            describe('#length', () => {
                it('always returns 1', () => {
            ...
            describe('#listen()', () => {
                it('does nothing and logs a warning', () => {
                    ...
                    expect(stubbedLogger.warn).toHaveBeenCalledWith('history.listen() is not supported.');
                });
            ...
            describe('#createHref()', () => {
                it('returns the href for the given location based on the static root history', () => {
                    ...
                    expect(href).toBe(createUrl({ test1: '/foo', test2: '/bar?a=b' }, { pathname: '/example', relative: true }));
                });
                it('normalizes the pathname', () => {
                    history1.push('/foo');
                    const location = { pathname: 'bar' };
                    const href = history2.createHref(location);
                    expect(href).toBe(createUrl({ test1: '/foo', test2: '/bar' }, { pathname: '/example', relative: true }));
                });
            });
        });
