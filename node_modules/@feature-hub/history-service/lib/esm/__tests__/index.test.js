...
import { createRootLocationTransformer, defineHistoryService, } from '..';
import { consumerPathsQueryParamName, createSearch, createUrl, } from './root-location-helpers';
    ...
    describe('HistoryServiceV1', () => {
        ...
        beforeEach(() => {
            ...
            createHistoryServiceBinder = () => {
                const sharedHistoryService = defineHistoryService(createRootLocationTransformer({ consumerPathsQueryParamName })).create(mockEnv);
        ...
        describe('#createBrowserHistory()', () => {
            ...
            describe('when called multiple times for the same consumer', () => {
                it('returns the same instance and logs a warning', () => {
                    ...
                    expect(stubbedLogger.warn).toHaveBeenCalledWith('createBrowserHistory was called multiple times by consumer "test1". Returning the same history instance as before.');
                });
            });
            describe('#length', () => {
            ...
            describe('#push()', () => {
                it('pushes a new root location onto the root history for every consumer push', () => {
                    ...
                    expect(window.location.href).toBe(createUrl({ test1: '/foo', test2: '/bar?baz=1' }));
            ...
            describe('#go()', () => {
                it('does nothing and logs a warning that go() is not supported', () => {
                    ...
                    expect(stubbedLogger.warn).toHaveBeenCalledWith('history.go() is not supported.');
                });
            });
            describe('#goBack()', () => {
                it('does nothing and logs a warning that goBack() is not supported', () => {
                    ...
                    history1.goBack();
                    expect(window.location.href).toBe(href);
                    expect(stubbedLogger.warn).toHaveBeenCalledWith('history.goBack() is not supported.');
                });
            });
            describe('#goForward()', () => {
                it('does nothing and logs a warning that goForward() is not supported', () => {
                    ...
                    history1.goForward();
                    expect(window.location.href).toBe(href);
                    expect(stubbedLogger.warn).toHaveBeenCalledWith('history.goForward() is not supported.');
                });
            });
            describe('#block()', () => {
                it('does nothing and logs a warning', () => {
                    ...
                    expect(stubbedLogger.warn).toHaveBeenCalledWith('history.block() is not supported.');
                });
            ...
            describe('#createHref()', () => {
                it('returns the href for the given location based on the root browser history', () => {
                    ...
                    expect(href).toBe(createUrl({ test1: '/foo', test2: '/bar?a=b' }, { relative: true }));
                });
                it('normalizes the pathname', () => {
                    history1.push('/foo');
                    const location = { pathname: 'bar' };
                    const href = history2.createHref(location);
                    expect(href).toBe(createUrl({ test1: '/foo', test2: '/bar' }, { relative: true }));
                });
            });
    ...
    describe('HistoryServiceV3', () => {
        ...
        beforeEach(() => {
            ...
            createHistoryServiceBinder = (getHistoryKey) => {
                const sharedHistoryService = defineHistoryService(createRootLocationTransformer({ consumerPathsQueryParamName }), { getHistoryKey }).create(mockEnv);
        ...
        describe('#history', () => {
            ...
            describe('#back()', () => {
                it('does nothing and logs a warning that back() is not supported', () => {
                    ...
                    history1.back();
                    expect(window.location.href).toBe(href);
                    expect(stubbedLogger.warn).toHaveBeenCalledWith('history.back() is not supported.');
                });
            });
            describe('#forward()', () => {
                it('does nothing and logs a warning that forward() is not supported', () => {
                    ...
                    history1.forward();
                    expect(window.location.href).toBe(href);
                    expect(stubbedLogger.warn).toHaveBeenCalledWith('history.forward() is not supported.');
                });
            });
            describe('#block()', () => {
                it('does nothing and logs a warning', () => {
