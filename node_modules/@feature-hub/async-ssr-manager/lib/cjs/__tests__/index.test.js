...
const use_fake_timers_1 = require("./use-fake-timers");
const queueMacroTask = setImmediate;
async function simulateAsyncOperation(result) {
    await new Promise(queueMacroTask);
    return result;
}
describe('defineAsyncSsrManager', () => {
    let mockEnv;
    let asyncSsrManagerDefinition;
    beforeEach(() => {
        asyncSsrManagerDefinition = (0, __1.defineAsyncSsrManager)({ timeout: 5 });
    });
    beforeEach(() => {
        ...
        describe('rendering', () => {
            describe('with an integrator as the only consumer', () => {
                it('resolves with the result of the given render function after the first render pass', async () => {
                    ...
                    const html = await (0, use_fake_timers_1.useFakeTimers)(async () => asyncSsrManager.renderUntilCompleted(mockRender));
            ...
            describe('with an integrator, and a consumer that schedules a rerender with a custom promise', () => {
                it('resolves with an html string after the second render pass', async () => {
                    const asyncSsrManagerIntegrator = asyncSsrManagerBinder('test:integrator').featureService;
                    const asyncSsrManagerConsumer = asyncSsrManagerBinder('test:consumer').featureService;
                    let renderPass = 0;
                    ...
                    const html = await (0, use_fake_timers_1.useFakeTimers)(async () => asyncSsrManagerIntegrator.renderUntilCompleted(mockRender));
            ...
            describe('with an integrator, and two consumers that both schedule a rerender in the first render pass', () => {
                it('resolves with an html string after the second render pass', async () => {
                    const asyncSsrManagerIntegrator = asyncSsrManagerBinder('test:integrator').featureService;
                    const asyncSsrManagerConsumer1 = asyncSsrManagerBinder('test:consumer:1').featureService;
                    const asyncSsrManagerConsumer2 = asyncSsrManagerBinder('test:consumer:2').featureService;
                    let renderPass = 0;
                    const mockRender = jest.fn(() => {
                        renderPass += 1;
                        if (renderPass === 1) {
            ...
            describe('with an integrator that renders two consumers independently, both scheduling a rerender in the first render pass', () => {
                it('resolves with an html string after the second render pass', async () => {
                    ...
                    const createMockRender = (consumer) => {
                        let renderPass = 0;
                        return jest.fn(() => {
                            renderPass += 1;
                            if (renderPass === 1) {
                    ...
                    const html = await (0, use_fake_timers_1.useFakeTimers)(async () => Promise.all([
            ...
            describe('with a consumer that schedules a rerender outside of `renderUntilCompleted`', () => {
                it('rejects with an error', async () => {
                    const asyncSsrManager = asyncSsrManagerBinder('test:consumer').featureService;
                    const mockRender = jest.fn(() => {
                        asyncSsrManager.scheduleRerender();
                    });
                    return expect((0, use_fake_timers_1.useFakeTimers)(async () => mockRender())).rejects.toEqual(new Error('Async SSR Manager: Can not call `scheduleRerender` outside of `renderUntilCompleted`.'));
                });
            });
            describe('when the given render function throws an error', () => {
                it('rejects with the error', async () => {
                    ...
                    return expect(asyncSsrManager.renderUntilCompleted(mockRender)).rejects.toEqual(mockError);
                });
            });
            describe('when rendering takes longer than the configured timeout', () => {
                it('rejects with an error after the configured timeout', async () => {
                    const asyncSsrManager = asyncSsrManagerBinder('test').featureService;
                    const mockRender = jest.fn(() => {
                        asyncSsrManager.scheduleRerender(new Promise(() => undefined));
                        return 'testHtml';
                    });
                    return expect((0, use_fake_timers_1.useFakeTimers)(async () => asyncSsrManager.renderUntilCompleted(mockRender), 5)).rejects.toEqual(new Error('Got rendering timeout after 5 ms.'));
                });
            });
            describe('when no timeout is configured', () => {
                beforeEach(() => {
                    asyncSsrManagerDefinition = (0, __1.defineAsyncSsrManager)();
                    asyncSsrManagerBinder =
                        asyncSsrManagerDefinition.create(mockEnv)['1.0.0'];
                });
                it('logs a warning', async () => {
                    const asyncSsrManager = asyncSsrManagerBinder('test').featureService;
                    const mockRender = jest.fn(() => 'testHtml');
                    await (0, use_fake_timers_1.useFakeTimers)(async () => asyncSsrManager.renderUntilCompleted(mockRender));
                    expect(stubbed_logger_1.stubbedLogger.warn.mock.calls).toEqual([
                        [
